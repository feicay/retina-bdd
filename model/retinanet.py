...
class Conv2dBlock(nn.Module):
    def __init__(self, inchannel, outchannel, kernel_size, stride=1, bn=1, activation='relu'):
        ...
        init.kaiming_uniform_(self.module[0].weight.data, mode='fan_in')
    def forward(self, x):
        out = self.module(x)
        return out
...
class UpsampleAddBlock(nn.Module):
    def __init__(self, inchannel_left, outchannel):
        ...
        init.kaiming_uniform_(self.left[0].weight.data, mode='fan_in')
...
class ResidualLayer(nn.Module):
    def __init__(self, inchannel, outchannel):
        ...
        init.kaiming_uniform_(self.left[0].weight.data, mode='fan_in')
        init.kaiming_uniform_(self.left[3].weight.data, mode='fan_in')
    def forward(self, x):
        y = self.left(x) + x
        return F.relu(y)
...
class ResidualBlock(nn.Module):
    def __init__(self, inchannel, outchannel, block_num):
        ...
        init.kaiming_uniform_(self.layer1[0].weight.data, mode='fan_in')
    def forward(self, x):
        y = self.layer1(x)
        y = self.layer2(y) 
        return y
